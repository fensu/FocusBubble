...
- 鼠标速度自适应（速度 EMA：抬升 0.35 / 回落 0.06，3600 px/s 满速，smoothstep 缓动）：
  - dim ×(1−0.6·ease)（高速时外围更亮）
  - radius / band ×(1+0.8·ease)（清晰区临时扩大）
  - feather +260·ease（过渡区临时加宽）
  - blur ×(1−0.5·ease)
  - 跟随系数 ×(1−0.7·ease)（高速时跟随更慢）
  - 鼠标停止后参数逐渐收拢（回落慢的 EMA 保证）。
